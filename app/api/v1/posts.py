...
from app.model.post_model import (
    CreatePostReq,
    CreatePostResp,
    DeletePostResp,
...
@router.get("/posts/{post_id}", response_model=GetPostByIdResp)
def get_post_by_id(
    post_id: str,
    ...
    try:
        post = post_service.get_post_by_id(
            context=ctx, post_id=post_id, include_deleted=include_deleted
        )
    except DatabaseError as err:
        raise HTTPException(status_code=503, detail=str(err)) from err
...
@router.patch("/posts/{post_id}", response_model=UpdatePostResp)
def update_post(
    req: UpdatePostReq,
    post_id: str,
    ctx: Context = Depends(get_context),
    ...
    try:
        updated_post = post_service.update_post(
            context=ctx, post_id=post_id, title=req.title, content=req.content
        )
    except DatabaseError as err:
        raise HTTPException(status_code=503, detail=str(err)) from err
    except NotFoundError as err:
...
@router.delete("/posts/{post_id}", response_model=DeletePostResp)
def delete_post(
    post_id: str,
    ctx: Context = Depends(get_context),
    post_service: PostService = Depends(),
    ...
    try:
        deleted_post = post_service.delete_post(context=ctx, post_id=post_id)
    except DatabaseError as err:
        raise HTTPException(status_code=503, detail=str(err)) from err
    except NotFoundError as err:
        raise HTTPException(status_code=404, detail=str(err)) from err
