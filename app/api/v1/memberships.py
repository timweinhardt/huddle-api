...
def create_membership(
    ...
    try:
        new_membership = membership_service.create_membership(
            user_id=req.user_id, location_id=location_id, roles=req.roles
        )
...
@router.get(
    "/locations/{location_id}/memberships/{user_id}", response_model=MembershipResp
)
def get_membership(
    location_id: str,
    user_id: str,
...
def update_membership(
    ...
    try:
        updated_membership = membership_service.update_membership_roles(
            user_id=user_id, location_id=location_id, roles=req.roles
        )
    except DatabaseError as err:
        raise HTTPException(status_code=503, detail=str(err)) from err
    except NotFoundError as err:
...
@router.get(
    "/locations/{location_id}/memberships", response_model=GetLocationMembershipsResp
)
def get_location_memberships(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=str(err)) from err
    return GetLocationMembershipsResp(
        location_id=location_id, memberships=location_memberships
    )
